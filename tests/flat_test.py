...
class BeamLoader(object):
    ...
    def update_qs(self, step=1.):
        ...
        pdotp = numexpr.evaluate('self.px**2+self.py**2+self.pz**2')
        denom = np.sqrt(pdotp + rest_mass_sqrd)

        self.x += numexpr.evaluate('step* self.h * self.px / denom')
        self.y += numexpr.evaluate('step* self.h * self.py / denom')
        self.z += numexpr.evaluate('step* self.h * self.pz / denom')
    ...
    def update_ps(self):
        ...
        self.px += numexpr.evaluate('self.h*q*(self.beta_gamma/self.gamma)*self.Q*f_dx')
        self.py += numexpr.evaluate('self.h*q*(self.beta_gamma/self.gamma)*self.Q*f_dy')
        self.pz += numexpr.evaluate('self.h*q*(self.beta_gamma/self.gamma)*self.Q*f_dz')
    ...
    def rotate_fields(self, step=1.):
        ...
        # should speed test this against matrix multiplication using np.dot

        self.P = numexpr.evaluate('currentP*np.cos(self.OMEGA * step * self.h) - (1./self.OMEGA)*currentQ*np.sin(self.OMEGA* step * self.h)')

        self.Q = numexpr.evaluate('currentP *self.OMEGA* np.sin(self.OMEGA * step * self.h) + currentQ * np.cos(self.OMEGA * step * self.h)')
    ...
    def update_Ps(self):
        ...
        # Assume only one mode for now!
        self.P += numexpr.evaluate('self.h * q * (self.beta_gamma/self.gamma)* np.sum(fz)')
